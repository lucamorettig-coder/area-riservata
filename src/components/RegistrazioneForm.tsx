...
export default function RegistrazioneForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/registrazione`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        if (data.errors) {
          ...
          setSubmitError(data.error || 'Errore durante la registrazione');
        }
      } else {
        // Redirect al login con parametro di successo
        window.location.href = `${baseUrl}/login?registered=true`;
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ padding: '1rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        ...
        <p className="text-sm text-muted-foreground">Compila tutti i campi per creare il tuo account</p>
      </div>
