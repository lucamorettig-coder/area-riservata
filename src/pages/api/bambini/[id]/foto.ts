...
export const POST: APIRoute = async (context) => {
  try {
    console.log('[Foto API] Starting POST request');
    
    // Verifica autenticazione
    const genitore = await getGenitoreFromSession(context);
    console.log('[Foto API] Genitore found:', !!genitore);
    
    if (!genitore) {
      console.error('[Foto API] Not authenticated');
    ...
    console.log('[Foto API] Bambino ID:', bambinoId);
    ...
    if (!client) {
      console.error('[Foto API] Airtable client not configured');
    ...
    console.log('[Foto API] Bambino found:', !!bambino);
    
    if (!bambino) {
      console.error('[Foto API] Bambino not found or not owned by genitore');
    ...
    try {
      ...
      console.log('[Foto API] JSON body parsed successfully');
    } catch (e) {
      console.error('[Foto API] Error parsing JSON:', e);
    ...
    console.log('[Foto API] File present:', !!fileData);
    console.log('[Foto API] File name:', fileName);
    console.log('[Foto API] File type:', fileType);
    console.log('[Foto API] File size:', fileSize);

    if (!fileData) {
      return new Response(JSON.stringify({ error: 'Carica un\'immagine' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!fileType || !fileType.startsWith('image/')) {
      console.error('[Foto API] Invalid file type:', fileType);
      return new Response(JSON.stringify({ error: 'Formato file non supportato. Usa JPG, PNG o GIF.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (fileSize > maxSize) {
      console.error('[Foto API] File too large:', fileSize);
    ...
    const r2Key = `foto-bambini/${bambinoId}/${timestamp}-${sanitizedFileName}`;

    console.log('[Foto API] Uploading to R2:', r2Key);
    ...
    console.log('[Foto API] File uploaded to R2 successfully');
    ...
    console.log('[Foto API] Public URL:', publicUrl);

    // Aggiorna Airtable con l'URL pubblico
    const updated = await client.updateFotoBambino(
      bambinoId,
      genitore.id!,
      publicUrl
    );
    ...
    if (!updated) {
      console.error('[Foto API] Failed to update Airtable');
    ...
    console.log('[Foto API] Photo uploaded successfully');
    return new Response(JSON.stringify({ 
      success: true,
    ...
    console.error('[Foto API] Unexpected error:', error);
    return new Response(JSON.stringify({ 
